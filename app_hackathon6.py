...
<AirVolumeScreen>:
    ...
    client_count_label: client_count_label
    name: "AirVolume"
    FloatLayout:
        canvas.before:
            Color:
...
<AirVolumeClientScreen>:
    status_label: status_label
    volume_status_label: volume_status_label
    ip_input: ip_input
    port_input: port_input
    name: "AirVolumeClient"
    FloatLayout:
        canvas.before:
            Color:
                rgba: 0.05,0.05,0.05,1
        ...
        BoxLayout:
            ...
            MDRaisedButton:
                ...
                size_hint_x: 0.5
                font_size: "16sp"
                on_release: app.connect_air_volume_client()
            ...
            MDRaisedButton:
                ...
                size_hint_x: 0.5
                font_size: "16sp"
                on_release: app.disconnect_air_volume_client()
        ...
        MDRaisedButton:
            ...
            font_size: "14sp"
            on_release: app.go_back_to_profile_from_air_volume_client()
    ...
    FloatLayout:
        ...
        BoxLayout:
            ...
            MDRaisedButton:
                ...
                text_color: 1,1,1,1
                size_hint_y: 0.2
                on_release: app.AirVolumeClient()
...
class AirVolumeScreen(Screen):
    ...
    client_count_label = ObjectProperty(None)


class AirVolumeClientScreen(Screen):
    status_label = ObjectProperty(None)
    volume_status_label = ObjectProperty(None)
    ip_input = ObjectProperty(None)
    port_input = ObjectProperty(None)
...
class HandServerApp(MDApp):
    def build(self):
        ...
        # Air Volume Server variables - FIXED
        self.air_server_socket = None
        self.air_clients = []  # List to store multiple clients
        self.air_server_running = False
        self.air_server_thread = None

        # Air Volume Client variables
        self.air_volume_client_socket = None
        self.air_volume_client_connected = False
        self.air_volume_client_running = False
        self.air_volume_client_thread = None
    ...
    def AirVolume(self):
        print("Air Volume Server clicked - showing AirVolume screen")
        self.root.current = "AirVolume"

    def AirVolumeClient(self):
        print("Air Volume Client clicked - showing AirVolumeClient screen")
        self.root.current = "AirVolumeClient"
    ...
    # --- Air Volume Server (FIXED - Camera starts immediately) ---
    def start_air_volume_server(self):
        ...
        if not self.air_capture.isOpened():
            self.update_air_status("Failed to open camera")
            return
        ...
        self.air_server_thread = Thread(target=self.run_air_volume_server,
                                        args=(server_ip, server_port), daemon=True)
        self.air_server_thread.start()

        self.update_air_status(f"Server starting on {server_ip}:{server_port}...")
    ...
    def run_air_volume_server(self, server_ip, server_port):
        try:
            ...
            self.air_server_socket.listen(5)
            self.air_server_socket.settimeout(1.0)

            Clock.schedule_once(
                lambda dt: self.update_air_status(
                    f"Server running on {server_ip}:{server_port} - Waiting for clients..."))
            print(f"Server running on {server_ip}:{server_port} - Waiting for clients...")

            # Continuously accept new clients
            while self.air_server_running:
                try:
                    client_socket, addr = self.air_server_socket.accept()
                    print(f"New client connected from {addr}")
                    self.air_clients.append(client_socket)

                    # Update client count
                    Clock.schedule_once(
                        lambda dt, count=len(self.air_clients): self.update_client_count(count))
    ...
    def update_client_count(self, count):
        if self.root and self.root.current == "AirVolume":
            ...
            if screen and hasattr(screen, 'client_count_label'):
                screen.client_count_label.text = f"Connected Clients: {count}"
    ...
    def update_air_volume_server(self, dt):
        ...
        if results.multi_hand_landmarks:
            ...
            for client in self.air_clients:
                try:
            ...
            for client in disconnected_clients:
                self.air_clients.remove(client)
    ...
    # --- Air Volume Client (Your exact code) ---
    def connect_air_volume_client(self):
        if self.air_volume_client_connected:
            return

        try:
            server_ip = self.root.get_screen("AirVolumeClient").ip_input.text
            server_port = int(self.root.get_screen("AirVolumeClient").port_input.text)

            if server_ip == "":
                self.update_air_volume_client_status("Please enter IP address", (1, 0, 0, 1))
                return

            if server_port < 1024 or server_port > 65535:
                self.update_air_volume_client_status("Port must be 1024-65535", (1, 0, 0, 1))
                return

        except ValueError:
            self.update_air_volume_client_status("Invalid port number", (1, 0, 0, 1))
            return
        except:
            self.update_air_volume_client_status("Invalid IP or Port", (1, 0, 0, 1))
            return

        self.air_volume_client_thread = Thread(target=self.run_air_volume_client,
                                               args=(server_ip, server_port), daemon=True)
        self.air_volume_client_thread.start()

    def run_air_volume_client(self, server_ip, server_port):
        """Your exact client code for AirVolume"""
        try:
            Clock.schedule_once(lambda dt: self.update_air_volume_client_status(
                f"Connecting to {server_ip}:{server_port}...", (1, 0.5, 0, 1)))

            self.air_volume_client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.air_volume_client_socket.connect((server_ip, server_port))

            self.air_volume_client_connected = True
            self.air_volume_client_running = True

            Clock.schedule_once(lambda dt: self.update_air_volume_client_status(
                "Connected to server", (0, 1, 0, 1)))
            Clock.schedule_once(lambda dt: self.update_air_volume_client_volume_status("Volume: Normal"))
            print(f"Connected to AirVolume server {server_ip}:{server_port}...")
            ...
            while self.air_volume_client_running:
                try:
                    data = self.air_volume_client_socket.recv(4096)
                    if not data:
                        break
                    ...
                    if length > MAX_DISTANCE:
                        ...
                        Clock.schedule_once(lambda dt: self.update_air_volume_client_volume_status("Volume: UP"))
                        print("Volume Up")
                        time.sleep(KEY_DELAY)
                    elif length < MIN_DISTANCE:
                        pyautogui.press("volumedown")
                        Clock.schedule_once(lambda dt: self.update_air_volume_client_volume_status("Volume: DOWN"))
                        print("Volume Down")
                        time.sleep(KEY_DELAY)
                    else:
                        Clock.schedule_once(lambda dt: self.update_air_volume_client_volume_status("Volume: Normal"))
            ...
            Clock.schedule_once(lambda dt, msg=error_msg: self.update_air_volume_client_status(
                f"Error: {msg}", (1, 0, 0, 1)))
        finally:
            self.disconnect_air_volume_client()

    def update_air_volume_client_status(self, text, color):
        if self.root and self.root.current == "AirVolumeClient":
            screen = self.root.get_screen("AirVolumeClient")
            if screen and hasattr(screen, 'status_label'):
                screen.status_label.text = text
                screen.status_label.color = color

    def update_air_volume_client_volume_status(self, text):
        if self.root and self.root.current == "AirVolumeClient":
            screen = self.root.get_screen("AirVolumeClient")
            if screen and hasattr(screen, 'volume_status_label'):
                screen.volume_status_label.text = text

    def disconnect_air_volume_client(self):
        self.air_volume_client_running = False
        self.air_volume_client_connected = False
        if hasattr(self, 'air_volume_client_socket') and self.air_volume_client_socket:
            try:
                self.air_volume_client_socket.close()
            except:
                pass
            self.air_volume_client_socket = None
        self.update_air_volume_client_status("Disconnected", (1, 0, 0, 1))
        self.update_air_volume_client_volume_status("Volume: Disconnected")
    ...
    def start_air_gesture_server(self):
        ...
        self.air_gesture_server_thread = Thread(target=self.run_air_gesture_server,
                                                args=(server_ip, server_port), daemon=True)
    ...
    def connect_air_gesture_client(self):
        ...
        self.air_gesture_client_thread = Thread(target=self.run_air_gesture_client,
                                                args=(server_ip, server_port), daemon=True)
    ...
    def go_back_to_profile_from_air_volume_client(self):
        self.disconnect_air_volume_client()
        self.root.current = "profile"
    ...
    def on_back_button(self, window, key, *args):
        ...
        if key == 27:  # Hardware back
            if current in ["server", "volumecamera", "volumeoption", "AirVolume", "AirVolumeClient", "AirGestureClient",
                           "GameSteering",
